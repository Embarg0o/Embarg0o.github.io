...
print(az_vocab[:10])
print(az_vocab[100:110])
print(az_vocab[1000:1010])
print(az_vocab[-10:])
...
# Посмотрим на фрагменты английского словаря:
...
# Проверим, что на диске создались 2 текстовых файла.
!ls *.txt

# Загрузка токенайзера из файла
# Теперь мы всегда можем загрузить токенайзер из ранее созданных файлов.
...
# Мы хотим, чтобы при детокенизации мы сразу получали строку, а не тензоры, а также 
# чтобы выходные строки не содержали зарезервированные токены. Для этого напишем вспомогательную функцию:
...
# Вот в таком виде по умолчанию выводятся детокенизированные строки:
...
# Создадим свой кастомный класс `CustomTokenizer` на базе `text.BertTokenizer`, с дополнительной пользовательской логикой, и `@tf.function` обертками, необходимых для экспорта.
...
# А модуль сохранить и обращаться к нему, когда он нам понадобиться.
model_name = 'ted_hrlr_translate_az_en_converter'
tf.saved_model.save(tokenizers, model_name)

# Теперь работать с токенизатором стало намного удобнее. 
# Именно данный подход позволяет создавать многоязыковые токенизаторы для мультиязыковых переводчиков.
# Пример токенизации:
...
for (az, en), en_labels in train_batches.take(1):
  break
...
embed_az = PositionalEmbedding(vocab_size=tokenizers.az.get_vocab_size().numpy(), d_model=512)
...
print(az_emb.shape)
print(en_emb.shape)
print(sample_ca(en_emb, az_emb).shape)
...
# Пропустим сигнал через слой `GlobalSelfAttention`  с двумя головами и посмотрим на размерности, 
# лишний раз убедившись, что выходная последовательность соответствует `query`. 
# Такие проверки на стадии разработки помогают избежать ошибок во время сбора модели целиком.

sample_gsa = GlobalSelfAttention(num_heads=2, key_dim=512)

print(az_emb.shape)
print(sample_gsa(az_emb).shape)
...
# Выходные данные для ранних элементов последовательности не зависят от более поздних элементов, 
# поэтому не должно иметь значения, обрезаете ли вы элементы до или после нанесения слоя (должно получиться очень маленькое число):
...
# Здесь `dff` - размерность первого `Dense` слоя, `d_model` - размерность эмбеддингов и нашего второго `Dense` слоя 
# (стараемся максимально близко по параметрам соответствовать оригинальной статье), `dropout_rate` - параметр регуляризации.
...
print(az_emb.shape)
print(sample_encoder_layer(az_emb).shape)
...
sample_encoder_output = sample_encoder(az, training=False)
...
# Таким образом, мы написали классы `Encoder` и `Decoder`. Чтобы получить модель `Transformer` целиком, 
# нам необходимо их соединить и добавить конечный `Dense` слой, который преобразует результирующий вектор в каждой позиции в вероятности выходных токенов.
...
# Пример 4: 
sentence = 'bu mənim etdiyim ilk kitabdır.'
